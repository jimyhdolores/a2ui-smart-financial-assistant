...
import { RouteDecision } from '../../services/gen-ui';
...
import { A2UI_REGISTRY } from '../../ui-catalog/component.registry';
...
export class Chat {
  ...
  protected readonly registry = A2UI_REGISTRY;
  ...
  /** Consultas de ejemplo (cubren las 6 intenciones del catálogo). */
  protected readonly suggestions: Suggestion[] = [
    { icon: 'pie_chart', label: '¿En qué gasto más?', query: '¿En qué estoy gastando más dinero este mes?' },
    { icon: 'emoji_nature', label: 'Mis gastos hormiga', query: '¿Qué gastos hormiga tengo y cómo van?' },
    { icon: 'qr_code_2', label: 'Solo mis Yape', query: 'Muéstrame únicamente los pagos que hice con Yape' },
    { icon: 'trending_up', label: 'Mi mayor compra', query: '¿Cuál fue mi compra más grande este mes?' },
    { icon: 'savings', label: '¿Cómo ahorro más?', query: '¿Cómo puedo ahorrar más dinero cada mes?' },
    { icon: 'credit_card', label: 'Celular a crédito', query: 'Quiero comprar un celular de $1200 a crédito en 12 cuotas, ¿me conviene?' },
  ];
  ...
  private async send(rawQuery: string): Promise<void> {
    ...
    try {
      // 1️⃣ Enrutado estructurado → qué componente + parámetros.
      ...
      // 2️⃣ La matemática la hace TypeScript, no el LLM.
      assistant.data = this.analytics.buildData(decision.componentToRender, decision.params, account);
      assistant.loading = false;
      assistant.streaming = true;
      this.commit();

      // 3️⃣ Narrativa en streaming (efecto máquina de escribir real).
